...
# Haphazardly adjusted and mangled by Pe8er (https://github.com/Pe8er) and then further mangled by Aaron (https://github.com/Aaron-Jose)
...
options =
  ...
  # Choose widget size.
  widgetSize: "medium"                 # work | medium | smol
...
update: (output, domEl) ->
  ...
  if !output
    ...
    isLovedcomma = isLoved.replace(/^\s*,|,\s*$/g, '')
    div.find('.heart').hide()
    ...
    if currArt isnt tArtwork and tArtwork isnt 'NA'
      ...
      tArtworkcomma = tArtwork.replace(/^\s*,|,\s*$/g, '')
      artwork.css('background-image', 'url('+tArtworkcomma+')')
